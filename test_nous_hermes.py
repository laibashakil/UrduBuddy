...
try:
    print("Loading Nous Hermes 2 Mistral model...")
    start_time = time.time()
    model = AutoModelForCausalLM.from_pretrained(
        "models/Nous-Hermes-2-Mistral-7B-DPO.Q4_K_M.gguf",
        model_type="llama",
        max_new_tokens=128,
        context_length=512,
        temperature=0.3
    ...
    for i, (question, expected) in enumerate(questions, 1):
        ...
        # Nous Hermes instruction format
        prompt = f"""<|im_start|>system
You are a helpful Urdu language assistant. Please read this story and answer the question in Urdu.
<|im_end|>
<|im_start|>user
Story: {story}
